        ...
        try:
            async with aiohttp.ClientSession() as session:
                try:
                    ...
                    real_proxy = {'http':'http://' + proxy}
                    print('Testing', proxy)
                    response = requests.get(url,headers=HEADERS,allow_redirects=False,proxies=real_proxy)
                    if response.status_code == 200:
                        self._conn.put(proxy)
                        print('success proxy', proxy)
                    else:
                        print('此ip被封，不可用',proxy)
                except (ProxyConnectionError, TimeoutError, ValueError) as s:
                    print('error proxy', s,proxy)
